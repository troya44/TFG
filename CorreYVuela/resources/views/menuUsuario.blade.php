...
<main class="container my-4">
    ...
    <section class="mb-5">
        ...
        <form action="{{ route('auth.actualizarPerfil') }}" method="POST">
            ...
            <div class="mb-3">
                ...
                <input type="text" name="apellido1" id="apellido1" value="{{ old('apellido1', $usuario->apellido1) }}" class="form-control" required>
                @error('apellido1')<small class="text-danger">{{ $message }}</small>@enderror
            </div>

            <div class="mb-3">
                <label for="apellido2" class="form-label">Segundo Apellido</label>
                <input type="text" name="apellido2" id="apellido2" value="{{ old('apellido2', $usuario->apellido2) }}" class="form-control" required>
                @error('apellido2')<small class="text-danger">{{ $message }}</small>@enderror
            </div>

            <div class="mb-3">
                <label for="localidad" class="form-label">Localidad</label>
                <input type="text" name="localidad" id="localidad" value="{{ old('localidad', $usuario->localidad) }}" class="form-control" required>
    ...
    {{-- Listado de carreras realizadas --}}
    <section>
        <h2>Mis Carreras</h2>
        @if($carreras->isEmpty())
            ...
            <ul class="list-group">
                @foreach($carreras as $carrera)
                    <li class="list-group-item">
                        <strong>{{ $carrera->nombre }}</strong> - {{ \Carbon\Carbon::parse($carrera->fecha)->format('d/m/Y') }}
                    </li>
                @endforeach
            </ul>
        @endif
